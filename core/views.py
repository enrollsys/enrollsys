    ...
    context.update(
        {
            ...
            "programs": StudyProgram.objects.filter(is_active=True).select_related(
                "department__faculty"
            )[:6],
...
def admission_schedule(request):
    exams = (
        Exam.objects.filter(is_active=True).select_related("program").order_by("date")
    )
    campaigns = AdmissionCampaign.objects.all()
    return render(
        request,
        "core/admission_schedule.html",
        {
            "exams": exams,
            "campaigns": campaigns,
...
def program_detail(request, pk):
    program = get_object_or_404(
        StudyProgram.objects.select_related("department__faculty"),
        pk=pk,
        is_active=True,
    )
...
def applicant_dashboard(request):
    ...
    ).select_related("exam", "application__program")
    return render(
        request,
        "core/applicant_dashboard.html",
        {
...
def applicant_applications(request):
    ...
    return render(
        request,
        "core/applicant_applications.html",
        {
            "applications": applications,
            "breadcrumbs": [
                ("Главная", "home"),
                ("Кабинет", "applicant_dashboard"),
                ("Мои заявки", None),
            ],
...
def application_detail(request, pk):
    application = get_object_or_404(
        Application.objects.select_related("program", "campaign"),
        pk=pk,
    ...
    return render(
        ...
        "core/application_detail.html",
        {
            ...
            "breadcrumbs": [
                ("Главная", "home"),
                ("Кабинет", "applicant_dashboard"),
                ("Мои заявки", "applicant_applications"),
                (f"Заявка #{application.pk}", None),
            ],
...
def methodist_dashboard(request):
    ...
    pending_count = all_apps.filter(status__in=["submitted", "under_review"]).count()
...
def methodist_applications(request):
    ...
    applications = Application.objects.all().select_related(
        "applicant", "program", "campaign"
    )
    return render(
        request,
        "core/methodist_applications.html",
        {
            "applications": applications,
            "breadcrumbs": [
                ("Главная", "home"),
                ("Кабинет методиста", "methodist_dashboard"),
                ("Заявки", None),
            ],
...
def methodist_documents(request):
    ...
    documents = ApplicationDocument.objects.select_related(
        "application__applicant", "application__program"
    )
    return render(
        request,
        "core/methodist_documents.html",
        {
            "documents": documents,
            "breadcrumbs": [
                ("Главная", "home"),
...
def application_review(request, pk):
    if not has_role(request.user, ["methodist", "admin"]):
        return redirect("home")
    application = get_object_or_404(
        Application.objects.select_related("applicant", "program", "campaign"), pk=pk
    )
    if request.method == "POST":
    ...
    return render(
        ...
        "core/application_review.html",
        {
            ...
            "breadcrumbs": [
                ("Главная", "home"),
                ("Заявки", "methodist_applications"),
                (f"Проверка #{application.pk}", None),
            ],
...
def document_upload(request, application_id):
    application = get_object_or_404(
        Application, pk=application_id, applicant=request.user
    )
    if request.method == "POST":
...
def exam_result_create(request):
    ...
    return render(
        request,
        "core/exam_result_form.html",
        {
            "form": form,
            "breadcrumbs": [
                ("Главная", "home"),
                ("Кабинет методиста", "methodist_dashboard"),
                ("Результат экзамена", None),
            ],
...
def export_application_docx(request, pk):
    ...
    if application.applicant != request.user and not has_role(
        request.user, ["methodist", "admin"]
    ):
    ...
    path = generate_docx(
        "Заявление абитуриента", content, f"application_{application.pk}.docx"
    )
...
def export_applications_xlsx(request):
    ...
    path = generate_xlsx(
        ["ID", "Абитуриент", "Email", "Код", "Направление", "Год", "Статус", "Создана"],
        rows,
        "applications_register.xlsx",
    )
    return serve_generated_file(path)
...
def verify_document(request, pk):
    """Mark a document as verified (methodist/admin only)."""
    if not has_role(request.user, ["methodist", "admin"]):
    ...
    messages.success(
        request, f"Документ «{document.get_doc_type_display()}» подтверждён."
    )
    referer = request.META.get("HTTP_REFERER", "")
    if referer:
        return redirect(referer)
    return redirect("methodist_documents")
...
def download_document(request, pk):
    ...
    if document.application.applicant != request.user and not has_role(
        request.user, ["methodist", "admin"]
    ):
...
def download_generated(request, filename):
    ...
    if (
        not filepath.exists()
        or generated_dir.resolve() not in filepath.resolve().parents
    ):
        raise Http404("Файл не найден.")
